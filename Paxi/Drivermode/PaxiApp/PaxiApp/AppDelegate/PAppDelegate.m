...
-(void)m_loadLoginView
{
    ...
    /* NavigationController Declaration*/
    UINavigationController *objNav=[[UINavigationController alloc]initWithRootViewController:loginVC];
...
#pragma mark - Location Updation Implementation-
-(void)FetchLocation
{
...
{
    if ([UIApplication sharedApplication].applicationState == UIApplicationStateBackground && [bgTimeStamp timeIntervalSinceNow]<-5*60)
    {
        bgTimeStamp=[NSDate date];
        // locationStarted=TRUE ;
    }
    if (locationStarted==TRUE )
    {
        ...
        NSLog(@"Application state=%d",[UIApplication sharedApplication].applicationState);
        
        NSLog(@"%d",abs(interval));
        if (currentLocation != nil && abs(interval)<60)
        {
            locationStarted=FALSE;
            [locationManager stopUpdatingLocation];
            ...
            timer = [NSTimer scheduledTimerWithTimeInterval:120 target:self selector:@selector(m_StartUpdatingLocation) userInfo:nil repeats:NO];
            
            NSString *strUserid=[[NSUserDefaults standardUserDefaults] valueForKey:userId];
            if (![strUserid isEqualToString:@""] && strUserid!=nil && [strUserid length]>0)
            {
                
                [[PApiCall sharedInstance]m_GetApiResponse:@"setLatLon" parameters:[NSString stringWithFormat:@"&userid=%@&lat=%.6f&lon=%.6f",[[NSUserDefaults standardUserDefaults] valueForKey:userId],currentLocation.coordinate.latitude,currentLocation.coordinate.longitude] onCompletion:^(NSDictionary *json)
                 {
                     NSLog(@"%@",json);
                }];
...
- (void)reverseGeocodeLocation
{
    ...
    [geocoder reverseGeocodeLocation:locationManager.location completionHandler:^(NSArray *placemarks, NSError *error) {
        ...
        if(placemarks.count){
            ...
            [[NSUserDefaults standardUserDefaults]setValue:[dictionary valueForKey:@"City"] forKey:@"locationName"];
            [[NSUserDefaults standardUserDefaults]synchronize];
        }
    }];
